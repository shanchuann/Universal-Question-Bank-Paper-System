...
public class UniversityDataService {
  ...
  public List<University> getUniversitiesByProvince(String province) {
    return UNIVERSITIES.values().stream().filter(u -> u.getProvince().equals(province)).toList();
  }
  ...
  public List<String> getAllProvinces() {
    return UNIVERSITIES.values().stream().map(University::getProvince).distinct().sorted().toList();
  }
