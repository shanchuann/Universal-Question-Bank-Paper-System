...
public class UserService {
  ...
  public UserEntity register(String username, String password, String role) {
    ...
    if (saved.getEmail() != null
        && !saved.getEmail().isBlank()
        && !Boolean.FALSE.equals(saved.getEmailNotification())) {
      String displayName =
          saved.getNickname() != null && !saved.getNickname().isBlank()
              ? saved.getNickname()
              : saved.getUsername();
      emailService.sendProfileUpdateNotification(saved.getEmail(), displayName);
    }
  ...
  public UserEntity updateProfile(
    ...
    if (systemNotification != null) {
      user.setSystemNotification(systemNotification);
    }
  ...
  public void resetPasswordByEmail(String email, String newPassword) {
    UserEntity user =
        userRepository.findByEmail(email).orElseThrow(() -> new RuntimeException("该邮箱未注册"));
